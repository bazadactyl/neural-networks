...
def main():
    ...
    for k in range(epochs):
        ...
        cv_start = (net.batch_size * k) % 60000
        cv_end = cv_start + net.batch_size
    ...
    print("\nFinished {} training epochs with {}-fold cross-validation!".format(epochs, num_folds))
    ...
    confidence = 0.95
    samples = test_error[-int(epochs/10):0] or test_error  # test set classification errors from last 10% of epochs
    mean = np.mean(samples)
    sigma = np.std(samples)
    interval = stats.norm.interval(confidence, loc=mean, scale=sigma)

    print("Classifier error rate using the 10,000 element test set: {:.2f}%".format(test_error[-1]))
