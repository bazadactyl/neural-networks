...
class YOLONet:
	...
	def fully_connected(self, input):
		with tf.variable_scope('fc1') as scope:
			...
			weights = _variable_with_weight_decay('weights', shape=[dim,?],
									 stddev=0.04,
									 wd=0.004)
	...
	def build(self, images):
		...
		with tf.variable_scope('fc') as scope:
			...
			weights = _variable_with_weight_decay('weights', shape=[dim,n],
									 stddev=0.04,
									 wd=0.004)
		...
		with tf.variable_scope('softmax') as scope:
			...
			weights = _variable_with_weight_decay('weights', shape=[n,7*7*30],
									 stddev=1/n,
									 wd=0.0)
